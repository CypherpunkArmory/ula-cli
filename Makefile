...
LDFLAGS=-ldflags "-X github.com/cypherpunkarmory/ulacli/restapi.apiVersion=$(CALVER) -X github.com/cypherpunkarmory/ulacli/cmd.version=$(VERSION) -X github.com/cypherpunkarmory/ulacli/cmd.rollbarToken=$(ROLLBAR_TOKEN) -s -w"
...
zip:
	...
	cd output; \
	echo "export const ulacliVersion = '$(VERSION)';" > release/version.js; \
	for f in ulacli*;  do \
		case "$$f" in \
			*.exe) extension=".exe" ;; \
			*) extension="" ;; \
		...
		filename="$${f%.*}"; \
		cp $${f} ulacli$${extension}; \
		zip release/$${filename}.zip ulacli$${extension};  \
		rm ulacli$${extension}; \
	done;
